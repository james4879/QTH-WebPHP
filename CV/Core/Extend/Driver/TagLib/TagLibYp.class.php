 ...
 */
if(defined('APP_NAME')!='Yourphp' && !defined("Chengming"))  exit("Access Denied");
			...
			if($r['type']==1){
				...
				foreach((array)$codes as $code){
					if($code){
						$codearr=explode("|",$code);
						$code= $codearr[0];$codename= $codearr[1];
						$datas.='<li><a href="msnim:chat?contact='.$code.'" target="_blank"><img src="'.__ROOT__.'/Public/Images/kefu/msn'.$skin.'.gif">'.$codename.'</a></li>';
					}
				}
			}elseif($r['type']==3){
				...
				foreach((array)$codes as $code){
					if($code){
						$codearr=explode("|",$code);
						$code= $codearr[0];$codename= $codearr[1];
						$datas.='<a target="_blank" href="http://www.taobao.com/webww/ww.php?ver=3&touid='.$code.'&siteid=cntaobao&status='.$skin.'&charset=utf-8" rel="nofollow" target="_blank"><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid='.$code.'&site=cntaobao&s='.$skin.'&charset=utf-8" alt="'.$r['name'].'" />'.$codename.'</a>';
					}
				}
			}elseif($r['type']==4){
				$codes=explode("\n",$r['code']);
