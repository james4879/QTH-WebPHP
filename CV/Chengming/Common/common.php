...
function make_urlrule($url,$lang,$action,$MOREREQUEST=''){
		...
		//by cheney url
		$REQUEST=str_replace(array('{$parentdir}','{$module}','{$moduleid}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$page}','{$title}'),array('','module','moduleid','catdir','year','month','day','catid','id',C('VAR_PAGE'),'title'),$matches[0]);
		$rule=str_replace(array('{$parentdir}','{$module}','{$moduleid}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$page}','/',C('URL_HTML_SUFFIX'),'{$title}'),array('','([A-Z]{1}[a-z]+)','(\d+)','([\w^_]+)','(\d+)','(\d+)','(\d+)','(\d+)','(\d+)','(\d+)','\/','','([\w^_]+)'),$url);
...
function URL($url='',$params=array()) { 
	...
	if(!empty($url)){
		...
		if(C('URL_M')==1)$index='/index.php/';
		if(C('URL_M') > 0){
			$url = $lang ? __ROOT__.'/index.php/'.$lang.'/' :  __ROOT__.'/';
		}else{
			$url = $lang ? __ROOT__.'/index.php?l='.$lang :  __ROOT__.'/';
		}
	}
...
function geturl($cat,$data='',$Urlrule=''){
		...
		if($id){
			if ($cat['module']){
				$title = M($cat['module'])->where("id=".$id)->getField('title');
				$title = $lang =='en' ? tourlstr($title):Pinyin($title);
			}
		}
		...
		if(empty($urls)){
			...
			if($id){
				...
				$urls = str_replace(array('{$parentdir}','{$module}','{$moduleid}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$title}'),array($parentdir,$module,$moduleid,$catdir,$year,$month,$day,$catid,$id,$title),$listurlrule);
			}
			$urls = explode('|',$urls);
