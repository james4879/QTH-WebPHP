...
class AjaxAction extends BaseAction
{
    ...
    public function getNews()
    {
		...
		$pagesiz =  !empty($this->Categorys[$id]['pagesize']) ? $this->Categorys[$id]['pagesize'] : C('PAGE_LISTROWS');
		$firstlist=$p*$pagesiz;
...
if($count){
	...
	foreach ($list as $k=>$v){
		$str.="<li><img src='".$v[an]."' class='news-list-first-bg'/>
            <a href='".$v[url]."'><img src='".$v[liang]."' /></a>
              <div class='news-hot'> <a href='".$v[url]."'>".$v[title]."</a> <span>".date('Y-m-d',$v[createtime])."</span>
                <p class='newsdesc'>".$v[description]."</p>
              	<div><a href='".$v[url]."' class='mores'></a>".$v[hits]."</div>
              </div>
            </li>";
	}
}
...
if($count){
	$list = $module->where($where)->order('listorder desc,createtime desc,id desc')->limit($firstlist,$pagesiz)->select();
	foreach ($list as $k=>$v){
		$str.="<li><img src='".$v[an]."' width='310' class='news-list-first-bg'/>
            <a href='".$v[url]."'><img src='".$v[liang]."' width='310'/></a>
              <div class='news-hot'> <a href='".$v[url]."'>".$v[title]."</a> <span>TYPE : ".$v[stype]."</span>
              </div>
            </li>";
	}
}
