...
class BaseAction extends Action
{
	...
	public function show($id='',$module='')
    {
		...
		if($module=="Article"){
			$pre = M($module)->where("id>$id and catid=$catid")->order('listorder asc,createtime asc,id asc')->find();
			$next = M($module)->where("id<$id and catid=$catid")->order('listorder desc,createtime desc,id desc')->find();
			$this->assign('pre',$pre);
	...
	public function down()
	{
		...
		if(strpos($filepath, ':/')) { 
			...
			$filepath = '.'.$filepath;
			//todo
			$filepath=str_replace(__ROOT__,'',$filepath);
