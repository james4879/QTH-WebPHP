...
class DatabaseAction extends AdminbaseAction{
	...
	public function cheneydocommand()
    {
        $tables = $_POST['tables'];
        $do= trim($_GET['do']);
	...
	public function cheneybackup(){
		...
		if($tableid < count($tables) || $r){
			...
			$forward = U("Database/cheneybackup",$urlarray);
			$this->assign ( 'jumpUrl', $forward);
			$this->success($message);
		 
		}else{ 
			S('backuptables',null);
			$this->assign ( 'jumpUrl', U(MODULE_NAME.'/cheneyrecover') );
			$this->success(L('do_ok'));	
		}
