...
class CategoryAction extends AdminbaseAction
{
    ...
    public function index()
    {
        ...
        if($this->categorys){
			foreach($this->categorys as $r) {
				...
				if(C('IS_ADD_CHILD')) $r['str_manage'] .='<a href="'.U('Category/add',array( 'parentid' => $r['id'],'type'=>$r['type'])).'">'.L('add_catname').'</a> | ';
				$r['str_manage'] .= '<a href="'.U('Category/edit',array( 'id' => $r['id'],'type'=>$r['type'])).'">'.L('edit').'</a>';
				//if(C('IS_DEL_CHILD')) $r['str_manage'] .=' | <a href="javascript:confirm_delete(\''.U('Category/delete',array( 'id' => $r['id'])).'\')">'.L('delete').'</a> ';
			...
			if (C('IS_FANGWEN'))
				$str  .= 	"<td align='center'>\$dis</td><td align='center'><a href='\$url' target='_blank'>".L('fangwen')."</a></td>";
			$str  .= 	"<td align='center'>\$str_manage</td>
					</tr>";
			import ( '@.ORG.Tree' );
			$tree = new Tree ($array);
			unset($array);
	...
	public function delete() {
		...
		if($this->categorys[$catid]['type']==1){
			...
			if($this->categorys[$catid]['moduleid']!=1 && $this->categorys[$catid]['moduleid']!=6 && $this->categorys[$catid]['moduleid']!=8){
