...
class AdminbaseAction extends Action
{
			...
			if($this->module[$this->moduleid]['type']==1 && ACTION_NAME=='index'){
				if($this->categorys){
					foreach ($this->categorys as $r){
						...
						if($_SESSION['groupid']!=1 && $_SESSION['groupid']!=2 && !in_array($_SESSION['groupid'],explode(',',$r['postgroup']))) continue;
	...
	function _list($modelname, $map = '', $sortBy = '', $asc = false ,$listRows = 15) {
		...
		if ($count > 0) {
			...
			$voList = $model->field($field)->where($map)->order( " listorder desc, `" . $order . "` " . $sort)->limit($page->firstRow . ',' . $page->listRows)->select ( );
	...
	public function create_show($id,$module)
    {
		...
		if(APP_LANG){
			...
			$T = F('config_'.LANG_NAME,'', './Chengming/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}else{
			L(include LANG_PATH.$this->sysConfig['DEFAULT_LANG'].'/common.php');
			$T = F('config_'.$this->sysConfig['DEFAULT_LANG'],'', './Chengming/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}
		$this->assign('T',$T);
		...
		if($CONTENT_POS !== false){
				...
				foreach ($pageurls as $p=>$urls) {
					...
					$this->buildHtml($filename,$dir,'./Chengming/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$template.C('TMPL_TEMPLATE_SUFFIX'));
				}
		}else{
		...
		if($cat['moduleid']==1){
			...
			if($CONTENT_POS !== false){
					...
					foreach ($pageurls as $p=>$urls) {
						...
						$r=$this->buildHtml($filename,$dir,'./Chengming/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Page_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
					}
			}else{
					$this->assign($data);
			...
			$r=$this->buildHtml($filename,$dir,'./Chengming/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$cat['module'].'_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
	...
	public function create_index($sitemap)
    {
		...
		if(empty($sitemap)){
			...
			$this->assign('bcid',0);
			$r=$this->buildHtml('index','./'.$lang,'./Chengming/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));
		}else{
			$this->assign('sitemap','1');
			$r=$this->buildHtml('sitemap','./'.$lang,'./Chengming/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
		}
		if($r) return true;
    }
