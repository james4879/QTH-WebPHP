...
class Page extends Think {
	...
	public function  myshow(){
		...
		$output = '';
		$output .= '<a href="'.$this->pageurl($urlrule, $pre_page,$this->parameter).'" class="page-prev"><</a>';
		$show_nums = $this->rollPage*2+1;// 显示页码的个数
	
		if($this->totalPages <= $show_nums){
			for($i = 1;$i<=$this->totalPages;$i++){
				if($i == $this->nowPage){
					$output .= '<a class="xuanzhong">'.$i.'</a>';
				}else{
					$output .= '<a href="'.$this->pageurl($urlrule,$i,$this->parameter).'">'.$i.'</a>';
				}
			}
		...
		$output .='<a href="'.$this->pageurl($urlrule,$next_page,$this->parameter).'" class="page-next">></a>';
		return $output;
	}
