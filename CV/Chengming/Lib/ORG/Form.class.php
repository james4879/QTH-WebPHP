...
class Form extends Think {
	...
	public function catid($info,$value){
		...
		foreach ($Category as $r){
				...
				if( ($this->isadmin && $_SESSION['groupid']!=2 && $_SESSION['groupid']!=1 && !in_array($_SESSION['groupid'],$postgroup)) ||  (empty($this->isadmin) && !in_array( cookie('groupid'),$postgroup)) ) continue;
		...
		if($value){
			...
			if(is_array($options)){
				foreach($options as  $r) {
						...
						$k = trim($v[1]);
						$optionsarr[$k] = $v[0];
						$data .='<div id="uplistd_'.$field.$i.'"><input type="text" size="50" class="input-text" name="'.$field.'[]" value="'.$v[0].'"  /> <input type="text" class="input-text" name="'.$field.'_name[]" value="'.$v[1].'" size="30" /> &nbsp;<a href="javascript:remove_this(\'uplistd_'.$field.$i.'\');">'.L('remove').'</a> </div>';
						$i++;
				}
			}
		}
